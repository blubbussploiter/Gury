...
void RBX::BrickColor::BrickAtlasMap::orderInAtlas(Color4 brickColor, SurfaceType surface)
{
	if (!isInAtlas(brickColor, surface))
	{
		...
		if (surface == Smooth)
		{
			orderedTexture.resize(32 + 5, 32 + 1, 4); /* plus 5, 5 */
		}
		else
		{
			orderedTexture.resize(64 + 5, 128 + 1, 4); /* plus 5, 5 */
		}
		...
		if (surface != Smooth)
		{
			...
			GImage::pasteSubImage(surfaceCanvas, surfaceTexture, 2, 0, 0, 0, surfaceTexture.width, surfaceTexture.height);
