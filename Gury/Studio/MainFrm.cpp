...
#define GURY_WNDTEXT "Gury3d - development build"
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndMainTools.SetWindowText(_T("Standard"));
	m_wndRunServiceTools.SetWindowText(_T("Test"));
	m_wndCameraTools.SetWindowText(_T("Camera"));
	m_wndStudioTools.SetWindowText(_T("Mouse"));
...
LRESULT CMainFrame::OnTabChange(WPARAM wp, LPARAM lp)
{
	...
	if (wp != -1 && child)
	{
		...
		{
			RBX::RBXManager::get()->setCurrentApplication(child->application);
			child->application->getDatamodel()->emptyExplorerWindow();
			child->application->getDatamodel()->fillExplorerWindow();
		}
	}
...
void CMainFrame::OnUpdateFrameTitle(BOOL bAddToTitle)
{
	...
	if (pChild &&  pChild->application && (int)pChild->application != -1)
	{
		if (bAddToTitle)
		{
			std::string text = pChild->application->appName;
