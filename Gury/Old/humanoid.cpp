...
void RBX::Humanoid::setLocalTransparency(float transparency)
{
    ...
    for (size_t i = 0; i < parent->children->size(); i++)
    {
        ...
        if (IsA<PartInstance>(child))
        {
            PartInstance* pv = toInstance<PartInstance>(child);
            float f;
...
void RBX::Humanoid::setHumanoidAttributes()
{
    if (getParent())
    {
        humanoidHead = getParent()->findFirstChild<PartInstance>("Head");
        humanoidRootPart = getParent()->findFirstChild<PartInstance>("Torso");
    }
}
...
bool RBX::Humanoid::isValid()
{
    ...
    {
        if (humanoidHead->primitive && humanoidRootPart->primitive)
        {
            return true;
        }
    }
...
bool RBX::Humanoid::isJoined()
{
    if (isValid())
    {
        ...
        return joints->areConnectedUnbuiltPrimitives(humanoidRootPart->primitive, humanoidHead->primitive) || joints->areConnectedBodies(humanoidRootPart->getBody(), humanoidHead->getBody());
    }
    return 0;
}
...
bool RBX::Humanoid::isGrounded()
{
    ...
    Vector3 v;
    getHumanoidGroundData(distanceFrom,v,v);
    return (distanceFrom < hipHeight);
}
...
void RBX::Humanoid::onStep()
{
    ...
    RBX::StandardOut::print(RBX::MESSAGE_INFO, "Hello");
...
RBX::PartInstance* RBX::Humanoid::getRightArm()
{
    if (!rightArm)
    {
        rightArm = getParent()->findFirstChild<PartInstance>("Right Arm");
    }
    return rightArm;
}
...
RBX::PartInstance* RBX::Humanoid::getLeftArm()
{
    if (!leftArm)
    {
        leftArm = getParent()->findFirstChild<PartInstance>("Left Arm");
    }
    return leftArm;
}
...
RBX::PartInstance* RBX::Humanoid::getRightLeg()
{
    if (!rightLeg)
    {
        rightLeg = getParent()->findFirstChild<PartInstance>("Right Leg");
    }
    return rightLeg;
}
...
RBX::PartInstance* RBX::Humanoid::getLeftLeg()
{
    if (!leftLeg)
    {
        leftLeg = getParent()->findFirstChild<PartInstance>("Left Leg");
    }
    return leftLeg;
}
