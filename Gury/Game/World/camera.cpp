...
{
    lookAtSelectableArray(SelectionService::get()->instancesAsSelection(SelectionService::get()->selection));
}

void RBX::Camera::lookAtSelectableArray(ISelectableArray array)
{
    Vector3 position;

    position = ISelectable::getCenterOfSelectableObjects(array);
...
bool RBX::Camera::canZoom(bool inout)
{
    ...
    if (cameraType == Follow && cameraSubject)
    {
        CoordinateFrame cameraFrame = camera->getCoordinateFrame();
        Ray ray;
...
{
    return RBX::Workspace::get()->getCurrentCamera();
}
