...
void RBX::pushVariant(lua_State* L, rttr::variant var)
{
	...
	if (var.can_convert<G3D::Vector3>())
	{
		...
		Lua::Bridge<G3D::Vector3>::pushNewObject(L, vector3);
		return;
	}
	...
	if (var.can_convert<G3D::CoordinateFrame>())
	{
		...
		Lua::Bridge<G3D::CoordinateFrame>::pushNewObject(L, cframe);
		return;
	}
	...
	if (type == rttr::type::get<Instances*>())
	{
		Instances* objects = var.convert<Instances*>();
		if (objects)
		{
			...
			{
				Lua::SharedPtrBridge<Instance>::pushObject(L, *iter);
				lua_rawseti(L, -2, i);
			}
